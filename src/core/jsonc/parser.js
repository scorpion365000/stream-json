...
// comma token (one per `,`, separator or trailing); emitted only when the
// `streamCommas` option is set, so round-trip editing can reproduce commas
const tokenComma = {name: 'comma'};
...
const jsoncParser = options => {
  ...
  if (options) {
    ...
    'streamCommas' in options && (streamCommas = options.streamCommas);
  }
  ...
  const handleComment = (tokens, index) => {
    ...
    if (cc === ASCII_SLASH) {
      // line comment: charCodeAt scan to CR/LF (the terminator is part of the token)
      let q = index + 2;
      while (q < len) {
      ...
      if (streamComments) tokens.push({name: 'comment', value: buffer.slice(index, q)});
      return q;
    }
    if (cc === ASCII_STAR) {
      ...
      for (let q = index + 2; q < len; ++q) {
        if (buffer.charCodeAt(q) === ASCII_STAR && q + 1 < len && buffer.charCodeAt(q + 1) === ASCII_SLASH) {
          if (streamComments) tokens.push({name: 'comment', value: buffer.slice(index, q + 2)});
          return q + 2;
        }
      }
    ...
    main: for (;;) {
      switch (expect) {
        ...
        case 'objectStop': {
          ...
          cc = buffer.charCodeAt(index);
          if (cc === ASCII_COMMA) {
            // Emit a `comma` token at the comma's position — before the trivia
            // that follows it, which the next state emits — so a round-trip
            // reproduces comma placement. The comma byte is already buffered, so
            // there is no lookahead and nothing to wait for (fully resumable).
            // Separator and trailing commas are handled uniformly: a trailing
            // comma is just a comma whose next non-trivia char is a close.
            if (streamCommas) tokens.push(tokenComma);
