...
test.asPromise('stringer: roundtrip', (t, resolve, reject) => {
  ...
  const pipeline = chain([readString(string), parser(), stringer()]);
...
test.asPromise('stringer: no packing', (t, resolve, reject) => {
  ...
  const pipeline = chain([readString(string), parser({packValues: false}), stringer()]);
...
test.asPromise('stringer: useValues', (t, resolve, reject) => {
  ...
  const pipeline = chain([readString(string), parser(), stringer({useValues: true})]);
...
test.asPromise('stringer: json streaming objects', (t, resolve, reject) => {
  ...
  const pipeline = chain([readString(string), parser({jsonStreaming: true}), stringer()]);
...
test.asPromise('stringer: json streaming objects as array', (t, resolve, reject) => {
  ...
  const pipeline = chain([readString(string), parser({jsonStreaming: true}), stringer({makeArray: true})]);
...
test.asPromise('stringer: no input as array', (t, resolve, reject) => {
  ...
  const pipeline = chain([readString(''), parser({jsonStreaming: true}), stringer({makeArray: true})]);
...
test.asPromise('stringer: control characters with useValues', (t, resolve, reject) => {
  const pipeline = chain([readString('{"a":"hello\\nworld\\t!\\u0000end"}'), parser(), stringer({useValues: true})]);
  let result = '';
